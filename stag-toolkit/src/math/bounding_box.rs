...
impl Mul<BoundingBox> for Mat4 {
    ...
    fn mul(self, rhs: BoundingBox) -> Self::Output {
        ...
        let min_bound = xa.min(xb).min(ya.min(yb).min(za.min(zb))).xyz();
        let max_bound = xa.max(xb).max(ya.max(yb).max(za.max(zb))).xyz();
...
mod tests {
    ...
    fn test_transform() {
        ...
        let rotated_90 = aabb;
        assert_eq!(rotate_90 * aabb, rotated_90, "rotating 45 degrees");
